...
const UploadPropertyPhotos = () => {
   ...
   const searchParams = useSearchParams()
    ...
    if(position=="Right" || position=="right"){
      imagePath = "uploads/propertyRightImgSmall/";
    }else if(position=="Left" || position=="left"){
      imagePath = "uploads/pImage/";
    }
    ...
    const deleteImageDatabase =(_id)=>{
      ...
      fetch(`${baseUrl}api/admin/auth/${uploadFor=="Property"?'uplopad-image-multiple':'upload-room-image'}`,{
        ...
        body:JSON.stringify({_id:_id, property_id:propertyId, position:position })
      }).then((response)=>{ 
        if(!response.ok){
        ...
        return response.json();
         ...
         $('.loaderouter').css('display','none');    
         if(data.status){
          setUploadedImages(uploadedImages.filter((pData) => pData._id !== _id));
  ...
  <div className="container-fluid">
    ...
    <div className="row">
      <div className="col-12">
        <div className="page-title-box d-sm-flex align-items-center justify-content-between">
          <h4 className="mb-sm-0 font-size-18">{uploadFor} {position} upload images</h4>
    ...
    <div className="row">
      ...
      <div className="col-lg-12">
        ...
        <div className="card">
          <div className="card-body">
            <h3> Uploaded {position} images</h3>
            <div className="row"> 
              <div className="col-lg-12">
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: '10px', marginTop: '20px' }}>
                {uploadedImages && uploadedImages.map((item, index) => (
                  <div key={index} style={{ display: 'inline-block', width: '100px', height: '100px', position:'relative' }}>
                    <img
                      src={`${baseUrl}${imagePath}${item.image}`}  
                      alt="img" 
                      width={'100%'}
                      height={'100%'}
