...
export async function POST(request) {
    ...
    const   {country_id, city_id, property_type_id, title, short_description, address, content, popular, amenities, Inclusive_bill, status ,longitude,latitude } = await request.json();

    
    const propertyDetail = new propertyDetails({
        country_id, city_id, property_type_id, title, short_description, address, content, popular, status,latitude,longitude,
...
export async function PUT(request) {
    ...
    const   {_id,country_id, city_id, property_type_id, title, short_description, address, content, popular, amenities, Inclusive_bill, status ,longitude,latitude } = await request.json();

    
    const propertyDetail =   await propertyDetails.findByIdAndUpdate({_id:new mongoose.Types.ObjectId(_id)},{
        country_id, city_id, property_type_id, title, short_description, address, content, popular, status,latitude,longitude,
        slug:slugify(title)+rand(11,99)
    })
    ...
    try{
        ...
        addAmenities(amenities,propertyDetail._id);
        addIncludeBill(Inclusive_bill,propertyDetail._id);
        return NextResponse.json({status:true, data:'Property has been updated successfully.', propertyDetail:propertyDetail});
    }catch(error){
        console.log('Error' ,error)
        return NextResponse.json({status:false, data:error});
    }
...
async function addAmenities(amenities, property_id){ 
    ...
    try{
        await propertyAmenities.deleteMany({property_id:new mongoose.Types.ObjectId(property_id)});
...
async function addIncludeBill(Inclusive_bill, property_id){ 
    ...
    try{
        await propertyIncludeBills.deleteMany({property_id:new mongoose.Types.ObjectId(property_id)});
